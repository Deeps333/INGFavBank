pipeline {
   agent any
	...
	stage ('Build')
		...
		{ steps {
                sh '/usr/share/maven/bin/mvn build-helper:parse-version versions:set -DnewVersion=\'${parsedVersion.nextmajorVersion}.${parsedVersion.minorVersion}.${parsedVersion.IncrementalVersion}\' -DgenerateBackupPoms=false'
	        sh 'sudo git add .'
